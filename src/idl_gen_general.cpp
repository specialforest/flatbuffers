...
BOOST_STATIC_ASSERT_MSG(sizeof(language_parameters) / sizeof(LanguageParameters) ==
        GeneratorOptions::kMAX, "Please add extra elements to the arrays above.");
...
static void GenStruct(const LanguageParameters &lang, const Parser &parser,
  ...
  for (std::vector<FieldDef *>::const_iterator it = struct_def.fields.vec.begin();
    ...
    const std::string &offset_prefix = ") { int o = __offset(" +
  ...
  if (struct_def.fixed) {
    ...
    for (std::vector<FieldDef *>::const_iterator it = struct_def.fields.vec.begin();
      ...
      if (field.value.type.base_type == BASE_TYPE_VECTOR) {
        ...
        std::size_t alignment = InlineAlignment(vector_type);
        std::size_t elem_size = InlineSize(vector_type);
...
static bool SaveClass(const LanguageParameters &lang, const Parser &parser,
  ...
  const std::string filename = namespace_dir + def.name + lang.file_extension;
...
bool GenerateGeneral(const Parser &parser,
  ...
  LanguageParameters lang = language_parameters[opts.lang];
